...
validate_script_paths() {
    ...
    for script in "${scripts[@]}"; do
        if [[ -f "$script" ]]; then
            ...
            if grep -q "^[[:space:]]*mods/" "$script" 2>/dev/null; then
                print_error "$script contains references to 'mods/' instead of 'minecraft/mods/'"
                ((errors++))
            fi
            
            # Check for correct minecraft/mods references
            if grep -q "minecraft/mods/" "$script" 2>/dev/null; then
                print_status "$script correctly references minecraft/mods/"
            fi
            ...
            if grep -q "config/" "$script" 2>/dev/null; then
                ...
                if grep -q "minecraft/config/" "$script" 2>/dev/null; then
                    print_status "$script correctly references minecraft/config/"
                else
                    print_warning "$script references config/ - verify this is correct for your use case"
                fi
            fi
        else
    ...
    if [[ -f "modrinth.index.json" ]]; then
        ...
        if command -v jq >/dev/null 2>&1; then
            ...
            if [[ -n "$mod_paths" ]]; then
                ...
                # Verify actual files exist in minecraft/mods/
