...
DIRECTORY STRUCTURE:
This script expects PrismLauncher directory structure:
  - minecraft/mods/           # Mod files
  - minecraft/config/         # Minecraft configuration
...
validate_structure() {
    ...
    if [[ -x "validate-directory-structure.sh" ]]; then
        ./validate-directory-structure.sh
    else
        print_error "validate-directory-structure.sh not executable"
        return 1
    fi
}
...
update_mods() {
    ...
    if [[ -x "update-mods.sh" ]]; then
        ./update-mods.sh $dry_run
    else
        print_error "update-mods.sh not executable"
        return 1
    fi
}
...
fix_data_validation() {
    ...
    if [[ -x "fix-data-validation-errors.sh" ]]; then
        ./fix-data-validation-errors.sh
    else
        print_error "fix-data-validation-errors.sh not executable"
        return 1
    fi
}
...
check_mod_dependencies() {
    ...
    if [[ -x "validate-dependencies.sh" ]]; then
        ./validate-dependencies.sh
    else
        print_error "validate-dependencies.sh not executable"
        return 1
    fi
}
...
$(if [[ -d "config" && -d "minecraft/config" ]]; then
    ...
    echo "✅ Configuration files: $root_configs (root), $mc_configs (minecraft)"
else
    echo "❌ Configuration directories not found"
fi)
