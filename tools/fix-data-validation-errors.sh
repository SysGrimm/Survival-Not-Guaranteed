...
validate_mod_compatibility() {
    ...
    if ! ls mods/ | grep -q -E "(create.*iron|create.*metal|create.*industrial)"; then
        # Check if we have recipes that might need ironworks functionality
        if find config/ -name "*.json" -exec grep -l "create.*iron\|ironworks" {} \; 2>/dev/null | head -1 >/dev/null; then
    ...
    if ! ls mods/ | grep -q -E "(tough.*nail|cold.*sweat|serene.*season|temperature|thirst)"; then
        # You have ColdSweat and SereneSeasons, so this check should pass
        if ls mods/ | grep -q -E "(ColdSweat|SereneSeasons|ThirstWasTaken)"; then
    ...
    if ! ls mods/ | grep -q -E "(farmers.*delight|farmers.*respite|cultural.*delight)"; then
        # You have FarmersDelight, so this should pass
        if ls mods/ | grep -q -E "(FarmersDelight|culturaldelights)"; then
...
fix_config_validation_errors() {
    ...
    local cold_sweat_at="mods/ColdSweat-2.4-b03c.jar"
