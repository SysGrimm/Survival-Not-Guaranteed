...
# This ensures script runs correctly regardless of where it's called from
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# If script is in tools/, parent is root. If in root, dir is root.
...
# Switch to project root for all operations
cd "$PROJECT_ROOT"
...
STRICT_EXTERNAL_DOWNLOADS="${STRICT_EXTERNAL_DOWNLOADS:-false}"
...
lookup_mod_with_mirrors() {
  ...
  # 3. Name-based search REMOVED
  # We should NOT guess files by name. If the hash lookup fails, it means the exact file isn't on Modrinth.
  # In that case, we should bundle the local version (via fallback below) rather than risking a wrong download.
...
create_mrpack() {
  ...
  if [ $PACK_INCLUDED -gt 0 ] && [ "$STRICT_EXTERNAL_DOWNLOADS" = "false" ]; then
    mkdir -p "$temp_dir/overrides/mods"
    local effective_mods_dir="$MODS_DIR"
    
    echo "  + Including unresolved mods in overrides/"
    ...
    for mod_file in "$effective_mods_dir"/*.jar; do
      ...
      # Check if this mod needs to be included (using simpler check to avoid re-hashing if possible, but validation is safer)
      # We know only the ones that failed "FOUND" need to be here, but we iterate all to match logic
      local lookup_result=$(lookup_mod_with_validation "$mod_file")
      local result_type=$(echo "$lookup_result" | cut -d'|' -f1)
      local source=$(echo "$lookup_result" | cut -d'|' -f2)
      
      if [ "$result_type" = "INCLUDE" ] || [ "$source" = "dependency-safety" ]; then
        cp "$mod_file" "$temp_dir/overrides/mods/"
